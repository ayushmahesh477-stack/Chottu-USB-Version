# ================================================================
# CHOTTU AI - AUTOMATED USB SETUP SCRIPT
# ================================================================
...
Write-Host "   CHOTTU AI - USB Setup                                    " -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Cyan
Write-Host ""
...
if ($HasCustom) {
    ...
    if ($customURL) {
        ...
        if ([string]::IsNullOrWhiteSpace($customPrompt)) {
            $customPrompt = "You are Chottu, a helpful AI assistant."
        }
...
New-Item -ItemType Directory -Force -Path "$USB_Drive\chottu_data" | Out-Null
...
# =================================================================
# AUTO-CONFIGURE CHOTTU TO USE EXTERNAL OLLAMA
# =================================================================
Write-Host ""
Write-Host "Configuring Chottu to use your models..." -ForegroundColor Yellow

$storageDir = "$USB_Drive\chottu_data\storage"
New-Item -ItemType Directory -Force -Path $storageDir | Out-Null

$firstModelLocal = $SelectedModels[0].Local
$envFilePath = "$storageDir\.env"

# Build the .env content for Chottu
$envContent = @"
LLM_PROVIDER=ollama
...
if (-Not (Test-Path $envFilePath)) {
    ...
    Write-Host "      Chottu configured to use: $firstModelLocal" -ForegroundColor Green
} else {
    ...
    if ($existing -match 'LLM_PROVIDER=ollama') {
        Write-Host "      Chottu already configured for Ollama." -ForegroundColor Green
    } else {
        # Overwrite with correct config (user was using built-in ollama)
        Set-Content -Path $envFilePath -Value $envContent -Force -Encoding UTF8
        Write-Host "      Chottu reconfigured to use external Ollama." -ForegroundColor Green
    }
}
...
if ($downloadErrors.Count -gt 0) {
    ...
    Write-Host "   SETUP COMPLETE! CHOTTU AI IS READY!                    " -ForegroundColor Green
    Write-Host "==========================================================" -ForegroundColor Cyan
}
...
Write-Host ""
Write-Host "  To start Chottu AI: Double-click  start-windows.bat" -ForegroundColor White
Write-Host "  On a Mac:            Double-click  start-mac.command" -ForegroundColor White
Write-Host ""
Write-Host "  TIP: In Chottu, go to Settings > LLM to switch" -ForegroundColor DarkGray
